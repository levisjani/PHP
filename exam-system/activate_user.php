...
<?php
	...
	if ($mode=='A')
	{
		$result1=mysql_query("update s_user_master set um_activate_date=sysdate() where um_id=".$id);
		header("location:activate_user.php"); 	

	}
	if ($mode=='D')
	{
		$result1=mysql_query("update s_user_master set um_activate_date=null where um_id=".$id);	
		header("location:activate_user.php"); 	
...
&nbsp;<?php
					...
					//get feedback
					$result = mysql_query("select * from s_user_master order by um_create_date desc");
				...
				if ($count>0)
				{
					...
					for($i=0;$i<$count; $i++)
					{
						...
						echo("<tr>");
							...
							{
								if (mysql_result($result,$i,"um_type")=='Researcher')
								{
