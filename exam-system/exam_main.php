...
<?php
	...
	if($_POST['BtnNext']=='  Next >  ')
	{
		...
		where q.q_sm_id=s.sm_id and s.sm_id=".$id."";
    //echo($id);
    $result=mysql_query($sql) or die(mysql_error());
		...
		//-----------------
			...
			$sql="SELECT * FROM s_user_survey  
			where us_um_id=".$uid." and us_sm_id=".$id."";
			//echo($sql);				
			...
			if ($row_count1<=0)
			{ 				 
				$sql="SELECT IFNULL(max(us_id)+1,1) m FROM s_user_survey  ";
				$result2=mysql_query($sql) or die(mysql_error());				
				$last_id=mysql_result($result2,0,"m");				
				//insert user survey
				$sql="insert into  s_user_survey values ($last_id,$id,$uid,sysdate())";
				//echo($sql);				
				$result1=mysql_query($sql) or die(mysql_error());							
				//insert user survey details
				$sql="SELECT IFNULL(max(usd_id)+1,1) m FROM s_user_survey_details  ";
				$result1=mysql_query($sql) or die(mysql_error());
				...
				{					
					...
					else
					{
					 ...
					 $sql="insert into  s_user_survey_details values ($last_detail_id,$last_id,$qm_id,$chk_dis,null)";//echo($sql);
					$result1=mysql_query($sql) or die(mysql_error());	
			...
			{				
				$usersurveyid=mysql_result($result1,0,"us_id");								
				$sql="delete FROM s_user_survey_details  
				where usd_us_id=".$usersurveyid." and usd_q_id=".$qm_id;				
				//echo($sql);
				$result1=mysql_query($sql) or die(mysql_error());
				...
				{					
					...
					else
					{
					 ...
					 $sql="insert into  s_user_survey_details values ($last_detail_id,$usersurveyid,$qm_id,$chk_dis,null)";//echo($sql);
					$result1=mysql_query($sql) or die(mysql_error());	
					//s.`usd_id`, s.`usd_us_id`, s.`usd_o_id`, s.`usd_o_select`, s.`usd_text` 
					$last_detail_id=$last_detail_id+1;					
				}				 
		...
		if($next_id> $row_count-1)
		{
			...
			header("location:survey_complete.php?id=1"); 
...
<?php 
		...
		//$sql="SELECT q.*, o.*, s.* FROM s_question_master q, s_question_options o, s_survey_master s 
    ...
    $sm_name=mysql_result($result,$next_id,"sm_name");
    //echo($sql);
    
    $sql="SELECT  * FROM s_question_options  
		where o_q_id=".$qm_id."";
		$result=mysql_query($sql) or die(mysql_error());
  	...
  	$link1="<img src='\\survey_images\\".$op_image1."'>";
  	$link2="<img src='\\survey_images\\".$op_image2."'>";
  	$link3="<img src='\\survey_images\\".$op_image3."'>";
  	$link4="<img src='\\survey_images\\".$op_image4."'>";
		...
		$sql2=" select us.*,usd.* from s_user_survey us, s_user_survey_details usd
...
<body>
<form name="form1"  method="post" action="survey_main.php?id=<?php echo($id)?>&next_id=<?php echo($next_id)?>" >
<div align="center">
