...
<?php
	...
	if($_POST['BtnSubmit']=='Submit')
	{
		...
		if ($mode=='add')
		{
		  ...
		  $sql="insert into question_master values($q_id,$sm_id,'$txt_question','$txt_type',$uid,sysdate(),$chk_dis) "; 
		  $result=mysql_query($sql) or die(mysql_error());
	    ...
	    $result=mysql_query($sql) or die(mysql_error()); 
			...
			//insert options
			$result1=mysql_query("select max(o_id)+1 m from question_options");	
		 	$o_id=mysql_result($result1,0,"m");
			//op 1
		 	$txt_opt=$_POST['TxtOption1'];
		 	$filename = stripslashes($_FILES['File1']['name']);		 	
			...
			$copied = copy($_FILES['image']['File1'], $newname); 	
		 	$txt_image=$filename;
			$sql="insert into question_options values($o_id,$q_id,'$txt_opt','$txt_image',$chk_op1)";$o_id=$o_id+1;
			$result=mysql_query($sql) or die(mysql_error());
			//op 2
		 	$txt_opt=$_POST['TxtOption2'];
		 	$filename = stripslashes($_FILES['File2']['name']);		 	
			...
			$copied = copy($_FILES['image']['File2'], $newname); 	
		 	$txt_image=$filename;
			$sql="insert into question_options values($o_id,$q_id,'$txt_opt','$txt_image',$chk_op2)";$o_id=$o_id+1;
			$result=mysql_query($sql) or die(mysql_error());			
			//op 3
		 	$txt_opt=$_POST['TxtOption3'];
		 	$filename = stripslashes($_FILES['File3']['name']);		 	
			...
			$copied = copy($_FILES['image']['File3'], $newname); 	
		 	$txt_image=$filename;
			$sql="insert into question_options values($o_id,$q_id,'$txt_opt','$txt_image',$chk_op3)";$o_id=$o_id+1;
			$result=mysql_query($sql) or die(mysql_error());
			//op 1
		 	$txt_opt=$_POST['TxtOption4'];
		 	$filename = stripslashes($_FILES['File4']['name']);		 	
			...
			$copied = copy($_FILES['image']['File4'], $newname); 	
		 	$txt_image=$filename;
			$sql="insert into question_options values($o_id,$q_id,'$txt_opt','$txt_image',$chk_op4)";$o_id=$o_id+1;
			$result=mysql_query($sql) or die(mysql_error());
...
<?php 
	...
	if ($mode=='edit')
	{
	    ...
	    $qm_type=mysql_result($result,0,"q_type");
	  	...
	  	$link1="<a href='\\survey_images\\".$op_image1."' target=_blank>".$op_image1."</a>";
	  	$link2="<a href='\\survey_images\\".$op_image2."' target=_blank>".$op_image2."</a>";
	  	$link3="<a href='\\survey_images\\".$op_image3."' target=_blank>".$op_image3."</a>";
	  	$link4="<a href='\\survey_images\\".$op_image4."' target=_blank>".$op_image4."</a>";
